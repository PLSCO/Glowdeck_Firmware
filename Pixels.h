...
#if defined(__AVR__) || defined(TEENSYDUINO) || defined(GLOWDECK)
#include <Arduino.h>
...
class PRGB
{
  public:
    ...
    PRGB(uint8_t r, uint8_t g, uint8_t b);
    PRGB();

    PRGB convert565toRGB(uint16_t color);
    uint16_t convertRGBto565(PRGB color);
...
class PixelsBase
{
  protected:
    ...
    void setCurrentPixel(PRGB* color);
    ...
    PRGB* computeColor(PRGB* bg, double weight);
    PRGB* computeColor(PRGB* fg, uint8_t opacity);
    ...
    PRGB* computedBgColor;
    PRGB* computedFgColor;
    PRGB* bgBuffer;
    PRGB* fgBuffer;
  ...
  public:
    ...
    */
    inline void setBackground(PRGB* color) {
        background = color;
    }
    /**
        Sets the current color to the specified color.
    ...
    */
    inline void setColor(PRGB* color) {
        foreground = color;
    }
    /**
    ...
    */
    PRGB* getPixel(int16_t x, int16_t y);
    /**
...
class BitStream
{
  ...
  public:
    ...
    bool endOfData() {
        return (unsigned)((bitpos + 1) >> 3) >= size;
    }
