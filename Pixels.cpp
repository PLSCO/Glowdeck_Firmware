...
PRGB::PRGB(uint8_t r, uint8_t g, uint8_t b)
{
    red = r;
    green = g;
    blue = b;
...
PRGB PRGB::convert565toRGB(uint16_t color)
{
...
uint16_t PRGB::convertRGBto565(PRGB color)
{
    return ((color.red / 8) << 11) | ((color.green / 4) << 5) | (color.blue / 8);
}

uint16_t PRGB::convertTo565()
{
...
PixelsBase::PixelsBase(uint16_t width, uint16_t height)
{
    ...
    computedBgColor = new PRGB(0, 0, 0);
    computedFgColor = new PRGB(0, 0, 0);
    bgBuffer = new PRGB(0, 0, 0);
    fgBuffer = new PRGB(0, 0, 0);
...
PRGB* PixelsBase::getPixel(int16_t x, int16_t y)
{
            ...
            for (int16_t i = 0; i < length - 8; i++)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        PRGB* cl = computeColor(fg, opacity);
                        setColor(cl);
                    }
...
PRGB* PixelsBase::computeColor(PRGB* bg, double alpha)
{
    if (alpha < 0)
    {
        alpha = 0;
...
PRGB* PixelsBase::computeColor(PRGB* fg, uint8_t opacity)
{
...
void PixelsBase::setCurrentPixel(PRGB* color)
{
